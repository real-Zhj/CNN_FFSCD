...
class RepVGG(nn.Module):
    ...
    def __init__(self, num_blocks, num_classes=1000, width_multiplier=None, override_groups_map=None, deploy=False, use_se=False, use_checkpoint=False):
        ...
        assert len(width_multiplier) == 4 # width_multiplier用于缩放模型各stage的通道数，如果该参数的列表长度不为 4，就会触发断言错误（AssertionError）
        self.deploy = deploy
...
# 张华建删改：in_channels从3改为4
        self.stage0 = RepVGGBlock(in_channels=4, out_channels=self.in_planes, kernel_size=3, stride=2, padding=1, deploy=self.deploy, use_se=self.use_se)
        ...
        for stage in (self.stage1, self.stage2, self.stage3, self.stage4):
            for block in stage:
                if self.use_checkpoint:
                    ...
                    out = block(out)
        #out = self.gap(out)                        #张华建删改：注释掉这一行
        #out = out.view(out.size(0), -1)            #张华建删改：注释掉这一行
        #out = self.linear(out)                     #张华建删改：注释掉这一行
        return out
...
def create_RepVGG_A00(deploy=False, use_checkpoint=False):
    return RepVGG(num_blocks=[1, 2, 4, 1], num_classes=1000,
